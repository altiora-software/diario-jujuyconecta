...
const RecentNewsList = ({ categorySlug }: { categorySlug?: string | null }) => {
  const [noticias, setNoticias] = useState<NotaResumida[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchRecent() {
      try {
        ...
        // Si recibimos categorySlug, primero buscamos el category_id
        if (categorySlug) {
          ...
          // Comportamiento original: últimas globales
          const { data, error } = await supabase
            .from("noticias")
            .select("id, titulo, slug, imagen_url, fecha_publicacion, created_at, estado")
            .eq("estado", "publicado")
